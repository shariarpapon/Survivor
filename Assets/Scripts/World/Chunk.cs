...
public class Chunk
{
    ...
    {
        if (Mathf.Sqrt(bound.SqrDistance(viewerPosition)) <= WorldManager.MAX_VIEW_DISTANCE) SetVisible(true);
        else SetVisible(false);
    }

    public bool IsChunkVisible(Vector3 viewerPosition)
    {
        if (Mathf.Sqrt(bound.SqrDistance(viewerPosition)) <= WorldManager.MAX_VIEW_DISTANCE) return true;
        else return false;
    }
