...
namespace Survivor
{
    namespace Core 
    {
        public class GameManager : MonoBehaviour
        {
            ...
            public static event System.Action<string, float> OnComponentInitialized;
            public static event System.Action OnGameInitilizationComplete;
            public static event System.Action OnGameStarted;
            public static event System.Action OnGameResumed;
            public static event System.Action OnGamePuased;
            ...
            {
                var initializers = FindObjectsOfType<MonoBehaviour>().OfType<IInitializer>();
            ...
            {
                var globalData = FindObjectsOfType<MonoBehaviour>().OfType<IGlobalStorableData>();
                foreach (var gd in globalData)
                    gd.SaveData();

                Debug.Log("<color=green>Global data succesfully saved...</color>");
            }

            public static string GetLocalGameDataDirectoryPath() => Application.persistentDataPath + $"/game_{GameIndex}";
            public static string GetLocalDataFilePath(string fileId) => GetLocalGameDataDirectoryPath() + $"/lcfdata_{fileId}.svd";
            public static string GetGlobalDataFilePath(string fileId) => Application.persistentDataPath + $"/gbfdata_{fileId}.svd";
        }
